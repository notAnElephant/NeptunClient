...
export default function HomeScreen() {
  ...
  const { nickname } = useNickname(profile.data?.name);
  ...
  return <Screen title={greetingFor(nickname || session?.userName || '')} action={<Pressable accessibilityLabel="Értesítések"><Ionicons name="notifications-outline" size={25} color={colors.navy} /></Pressable>} refreshing={calendar.isRefetching || exams.isRefetching || unread.isRefetching} onRefresh={refresh}>
