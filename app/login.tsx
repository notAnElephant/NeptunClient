  ...
  return <SafeAreaView style={styles.safe}><KeyboardAvoidingView style={styles.flex} behavior={Platform.OS === 'ios' ? 'padding' : undefined}><ScrollView contentContainerStyle={styles.content} keyboardDismissMode={Platform.OS === 'ios' ? 'interactive' : 'on-drag'} keyboardShouldPersistTaps="handled">
    ...
    {!isChallenge ? <>
      ...
      {isExternalLogin ? <View style={styles.externalNotice}><Ionicons name="shield-checkmark-outline" size={24} color={colors.blue} /><View style={styles.externalNoticeText}><Text style={styles.externalNoticeTitle}>Biztonságos ELTE bejelentkezés</Text><Text style={styles.externalNoticeHelp}>A Neptun-kódot, a jelszót és a minden belépéskor szükséges kétlépcsős kódot az ELTE saját oldalán adhatod meg.</Text></View></View> : <>
        ...
        <Text style={styles.label}>Jelszó</Text><View style={styles.input}><Ionicons name="lock-closed-outline" size={22} color={colors.muted} /><TextInput ref={passwordInputRef} value={password} onChangeText={setPassword} style={styles.textField} secureTextEntry={!passwordVisible} autoCapitalize="none" autoComplete="current-password" textContentType="password" returnKeyType="done" onSubmitEditing={submit} inputAccessoryViewID={Platform.OS === 'ios' ? LOGIN_KEYBOARD_ACCESSORY_ID : undefined} placeholder="Jelszó" placeholderTextColor={colors.muted} /><Pressable accessibilityRole="button" accessibilityLabel={passwordVisible ? 'Jelszó elrejtése' : 'Jelszó megjelenítése'} accessibilityState={{ expanded: passwordVisible }} hitSlop={8} onPress={() => setPasswordVisible((visible) => !visible)} style={({ pressed }) => [styles.passwordToggle, pressed && styles.passwordTogglePressed]}><Ionicons name={passwordVisible ? 'eye-off-outline' : 'eye-outline'} size={23} color={colors.muted} /></Pressable></View>
      </>}
...
const styles = StyleSheet.create({
  ...
  passwordToggle: { width: 32, height: 44, alignItems: 'center', justifyContent: 'center' }, passwordTogglePressed: { opacity: 0.5 },
