...
export default function LoginScreen() {
  const { session, authFlow, login, continueCaptcha, continueTwoFactor, resetAuthFlow } = useSession();
  const [institution, setInstitution] = useState<Institution>(institutions.find((item) => item.omCode === 'FI23344') ?? institutions[0]);
  const [userName, setUserName] = useState(''); const [password, setPassword] = useState('');
  const [challenge, setChallenge] = useState(''); const [pickerOpen, setPickerOpen] = useState(false); const [search, setSearch] = useState('');
  ...
  const submit = async () => {
    ...
    await login({ institution, userName, password });
  };
  ...
  return <SafeAreaView style={styles.safe}><KeyboardAvoidingView style={styles.flex} behavior={Platform.OS === 'ios' ? 'padding' : undefined}><View style={styles.content}>
    ...
    {!isChallenge ? <>
      ...
      <Text style={styles.help}>{authFlow.state === 'captchaRequired' ? 'Írd be a Neptun által megjelenített ellenőrző kódot.' : 'Írd be az intézményedtől kapott egyszer használatos kódot.'}</Text>
    ...
    <View style={styles.privacy}><Ionicons name="lock-closed-outline" size={20} color={colors.muted} /><Text style={styles.privacyText}>Adataid közvetlenül az intézményed Neptun rendszeréhez kerülnek.</Text></View>
...
const styles = StyleSheet.create({
  ...
  button: { height: 54, backgroundColor: colors.blue, borderRadius: radius.sm, alignItems: 'center', justifyContent: 'center', marginTop: spacing.xl }, buttonPressed: { opacity: 0.55 }, buttonText: { color: '#fff', fontSize: 17, fontWeight: '700' }, privacy: { flexDirection: 'row', gap: 12, marginTop: spacing.lg, paddingHorizontal: spacing.md }, privacyText: { flex: 1, color: colors.muted, fontSize: 14, lineHeight: 20 }, error: { color: colors.danger, marginTop: spacing.md, lineHeight: 20 },
